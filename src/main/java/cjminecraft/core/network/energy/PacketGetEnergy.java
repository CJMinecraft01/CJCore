...
public class PacketGetEnergy implements IMessage {
    ...
    public void fromBytes(ByteBuf buf) {
        try {
            ...
            if (this.updateFields) this.energyFieldName = ByteBufUtils.readUTF8String(buf);
            else this.modid = ByteBufUtils.readUTF8String(buf);
    ...
    public void toBytes(ByteBuf buf) {
        ...
        if (this.updateFields) ByteBufUtils.writeUTF8String(buf, this.energyFieldName);
    ...
    public static class Handler implements IMessageHandler<PacketGetEnergy, IMessage> {
        ...
        void processMessage(PacketGetEnergy message, MessageContext ctx) {
            ...
            if (te == null) return;
            if (!EnergyUtils.hasSupport(te, message.side)) return;
