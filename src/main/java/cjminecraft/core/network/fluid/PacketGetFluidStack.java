...
public class PacketGetFluidStack implements IMessage {
	...
	public PacketGetFluidStack() {
		this.messageValid = false;
	}
	
	public PacketGetFluidStack(int tankIndex, BlockPos pos, EnumFacing side, boolean updateFields, String... args) {
	...
	public static class Handler implements IMessageHandler<PacketGetFluidStack, IMessage> {
		
		@Override
		public IMessage onMessage(PacketGetFluidStack message, MessageContext ctx) {
		...
		void processMessage(PacketGetFluidStack message, MessageContext ctx) {
			...
			if(message.updateFields)
				PacketHandler.INSTANCE.sendTo(new PacketReturnFluidData(capacity, fluidStack, true, message.className, message.fluidStackFieldName), ctx.getServerHandler().player);
			else
